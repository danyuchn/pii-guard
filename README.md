...
- [ ] **更多文件類型**：library 端已能讀 docx/xlsx；把 Word、Excel 等格式接進本機網頁，並將不可信解析移進受限的私有子行程，驗證錯誤與警告不會回吐原文。
- [ ] **掃描／圖片型 PDF 與 OCR**：在本機辨識圖片中的文字，沿用檔案大小、頁數、時間與記憶體上限；原始頁面與辨識結果都不得送往外部服務，失敗時只回固定安全錯誤。
- [ ] **保留版面的 PDF 輸出**：研究依原座標重排或遮罩的做法，輸出可直接交付的去識別化 PDF；除了畫面看不到個資，也要檢查隱藏文字層、metadata 與附件沒有留下原值。

### 第五階段：可公開驗證與桌面交付

- [ ] **可公開的準確率**：另建不含私人資料、但接近真實文件分布的標註語料，公開 precision／recall／F1 與固定重跑方式，不拿私有驗收結果外推。
- [ ] **CI 自動守門**：每次變更自動執行測試、型別、格式與隱私邊界檢查；任何公開回應洩漏原文、私有對照表或路徑時不得通過。
