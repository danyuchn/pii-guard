...
```
客戶：王小明（身分證 A123456789，手機 0912345678）
公司：範例科技股份有限公司（統編 12345678）
地址：臺北市中正區測試路 100 號 5 樓
...
```
客戶：<PERSON_1>（身分證 <TW_NATIONAL_ID_1>，手機 <TW_MOBILE_1>）
公司：<ORG_1>（統編 12345678）
地址：<TW_ADDRESS_1> 100 號 5 樓
Email：<EMAIL_ADDRESS_1>
案由：本季顧問合約續約，金額新臺幣 30 萬元，付款分兩期。
```

**這份輸出有兩個瑕疵，我刻意不修掉它再貼上來：**

1. 統編 `12345678` 沒被遮蔽——八位純數字要靠上下文詞判斷，這次沒命中。
2. 地址只抓到 `臺北市中正區測試路`，`100 號 5 樓` 留在外面。

這正是為什麼下面那個 skill 在這條管線之後**還加了一層本地模型稽核**，以及為什麼有**人工補標介面**。單靠決定性偵測不夠，這是實話。

另外注意：金額、合約條款、日期都**留著**。這是刻意的——去掉它們，文件就沒有送進 AI 的意義了。
...
第 2 步會在**你的**瀏覽器開一頁，可以選取任何仍然裸露的文字補遮（全文所有出現位置一起遮），也可以點任何代號、看到背後的值、把它放回去——後者用在被誤判為組織而遮掉的法院、醫院、公司名。

那一頁**呼叫端連不上**：網址帶一次性 token，在私有子行程內產生、只交給它自己開的瀏覽器，從不印出；回執只有數量，沒有網址。每次編輯都即時落檔並重新驗證整份仍能逐字還原。
