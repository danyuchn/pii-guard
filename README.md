...
排第一不是因為最急，而是**它決定其他每一項的取捨空間**。稽核層目前開推理、每個視窗取樣三次，是因為它是最後一道防線，漏了就真的漏了。一旦有人工兜底，稽核就不必扛滿，效能優化的天花板才鬆得開。

兩個方向都要，而且反方向可能更有價值：

- **補遮**：使用者肉眼掃遮蔽後的檔找裸露的個資，標記，程式把該詞在全文所有出現位置一起遮掉。底層的 `_sweep_remaining_occurrences()` 已經支援，缺的是入口。
- **放回**：把被當成組織名遮掉的法院、醫院、公司名放回來。判決書遮完會失去部分可讀性正是這個原因。**這半邊零隱私風險**，而純文字檔操作這半邊特別痛苦，所以介面價值最高。

硬約束：**使用者標記的詞不得經過主 agent 視野**，否則等於把漏網的個資唸給雲端模型聽，隔離設計當場破功。因此介面必須是使用者直接操作本地程式，程式只回報數量。
