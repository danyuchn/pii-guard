...
- [x] **更多文件類型**：library 端已能讀 docx/xlsx；解析與二進位寫回移進有界私有行程，並以固定錯誤碼隔離第三方警告與例外。
- [x] **保留版面的 PDF 輸出**：以頁面尺寸相同的去識別化 raster PDF 輸出；無法安全遮蔽時 fail-closed。
- [x] **可公開的準確率**：版控內的 53 條合成標註語料附公開 manifest 與可重跑統計測試；不宣稱代表真實文件。
- [x] **CI**：GitHub Actions 安裝鎖定依賴，執行格式、lint、型別、非模型測試與公開合成評測。
