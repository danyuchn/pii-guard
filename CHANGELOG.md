...
- **加強稽核加入「控制個資」偵測指令壓制。** 每個送進本機模型的視窗前後各放一組文件裡不存在的
  合成聯絡人（姓名＋手機）；模型若漏報任一個，該次取樣以 `LOCAL_AUDIT_SUPPRESSED` 判定為被
  壓制而非「這段沒有個資」，並沿用既有的視窗切半重試把嵌入指令的段落隔離出來。控制值在對齊前
  剝除，不會進入私有對照表。原本只靠規則比對的中文注入偵測仍保留為便宜的前置過濾。
...
- **Windows 私有工作檔改為位元組保真。** 私有來源與去識別化檔的寫入端會把 LF 膨脹成 CRLF、
  ...
  Windows 上、且還原換行後雜湊完全一致時才接受載入，之後的提交會逐檔改寫成保真格式。
